...
def login():
    ...
    if request.method == 'POST':
        ...
        user = next(
            (user for user in users.values() if user.username == username),
            None
        )
...
class CVEDatabase:
    ...
    def search(self, package_name, version):
        ...
        for cve_id, cve_data in self.cves.items():
            ...
            if package_name.lower() in cve_data.get(
                'description',
                ''
            ).lower():
...
def analyze_services():
    ...
    for line in output.splitlines():
        ...
        if service in SERVICE_THREAT_RULES["known_services"]:
            ...
            family, severity = SERVICE_THREAT_RULES[
                "known_services"
            ][service]
...
def analyze_processes():
    ...
    for line in output.splitlines()[1:]:
        ...
        if mem >= 80:
            ...
            reasons.append(
                f"Extremely high Memory ({mem:.1f}%)"
            )

        elif mem >= 60:

            reasons.append(
                f"High Memory ({mem:.1f}%)"
            )
...
def analyze_users():
    ...
    users_output = run_command(
        "getent passwd | grep -v '^\\s*$'"
    )

    if "ERROR" in users_output:
        return {'error': users_output}

    sudo_output = run_command(
    ...
    sudo_users = (
        sudo_output.split(',')
        if sudo_output
        and not sudo_output.startswith("ERROR")
        else []
    )

    root_output = run_command(
        "getent passwd | "
        "awk -F: '$3==0 {print $1}'"
    )
    ...
    root_users = (
        root_output.split('\n')
        if root_output
        and not root_output.startswith("ERROR")
        else []
    )
    ...
    for line in users_output.split('\n'):
        ...
        if len(parts) >= 7:
            ...
            if shell in [
                ...
                reasons.append(
                    f'Unusual shell: {shell}'
                )

            if (
                home.startswith('/home/')
                or home.startswith('/export/home/')
            ):
                pass
...
def check_system_hardening():
    ...
    checks = [
...

        {
            ...
            'critical': True,
            'description':
                'Address Space Layout Randomization should be enabled'
        },
...

        {
            ...
            'critical': False,
            'description':
                'IPv4 redirects should be disabled'
        },
...
def generate_html_report(results, report_id):
    """Generate English HTML security report"""
    ...
    <head>
        ...
        <title>Sarabis Security Analysis Report</title>
    ...
    <body>
        ...
        <div class="header">
            ...
            <h1>🛡️ Sarabis Security Analysis Report</h1>
    ...
    if (
        'vulnerabilities' in results
        and 'summary' in results['vulnerabilities']
    ):

        vuln_summary = results[
            'vulnerabilities'
        ]['summary']
    ...
    if (
        'ports' in results
        and 'statistics' in results['ports']
    ):

        port_summary = results[
            'ports'
        ]['statistics']
    ...
    if (
        'services' in results
        and 'summary' in results['services']
    ):
        ...
        <div class="summary-card">
            ...
            <div class="number"
                ...
                {svc_summary.get('suspicious', 0)}

            </div>

            <div>Suspicious Services</div>
    ...
    if (
        ...
        and 'summary' in results['processes']
    ):

        proc_summary = results[
            'processes'
        ]['summary']
    ...
    if (
        'kernel_modules' in results
        and 'summary' in results['kernel_modules']
    ):

        kernel_summary = results[
            'kernel_modules'
        ]['summary']
        ...
        <div class="summary-card">
            ...
            <div class="number"
                ...
                {kernel_summary.get('suspicious', 0)}

            </div>

            <div>Suspicious Kernel Modules</div>
    ...
    if (
        'software' in results
        and 'summary' in results['software']
    ):
        ...
        <div class="summary-card">
            ...
            <div class="number"
                ...
                {sw_summary.get('suspicious', 0)}

            </div>

            <div>Suspicious Software</div>
    ...
    if (
        'users' in results
        and 'summary' in results['users']
    ):

        user_summary = results[
            'users'
        ]['summary']
    ...
    if (
        'vulnerabilities' in results
        and results['vulnerabilities'].get('cves')
    ):
    ...
    if suspicious_services:
        ...
        <div class="card">
            ...
            <div class="section-title">
                ...
                <h3>
                    ⚙️ Suspicious Services
                </h3>

                <span>
                    Total: {len(suspicious_services)}
                </span>
    ...
    if suspicious_processes:
        ...
        <div class="card">
            ...
            <div class="section-title">
                ...
                <span>
                    Total: {len(suspicious_processes)}
                </span>
        ...
        for proc in suspicious_processes[:20]:
    ...
    hardening = results.get(
        'hardening',
        {}
    )

    if hardening.get('checks'):

        summary = hardening.get(
            'summary',
            {}
        )
    ...
    html += f"""
        ...
        <div class="footer">
            ...
            <p>
                Generated by Sarabis Security Analyzer
            </p>

            <p style="font-size: 12px;">

                This report is for security analysis
                purposes only and does not make
                any changes to the analyzed system.
